...
import org.btcprivate.wallets.fullnode.daemon.ProvingKeyFetcher;
...
public class StartupProgressDialog extends JFrame {
    ...
    public void waitForStartup() throws IOException,
        ...
        Log.info("Splash: checking if btcpd is already running...");
        ...
        if (!shouldStartZCashd) {
            Log.info("Splash: btcpd already running...");
            ...
            public void run() {
                ...
                try
                {
                    ...
                    if (StartupProgressDialog.this.isAlive(daemonProcess)) {
                        Log.info("btcpd is still alive although we tried to stop it. " +
                                "Hopefully it will stop later!");
                        //System.out.println("btcpd is still alive, killing forcefully");
                        //daemonProcess.destroyForcibly();
                    } else
                        Log.info("btcpd shut down successfully");
                } catch (Exception bad) {
                    Log.error("Couldn't stop btcpd!", bad);
                }
            }
        });
